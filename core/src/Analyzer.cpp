...
extern "C" TSLanguage *
tree_sitter_typescript(); // ensure you build/link tree-sitter-typescript
...
nlohmann::json Report::to_json() const
{
    ...
    for (auto &d : diagnostics)
        j["diagnostics"].push_back(
            {{"rule", d.rule}, {"message", d.message}, {"line", d.line}});
    return j;
}
...
static int byte_to_line(const std::string &s, size_t byte_index)
{
    int line = 1;
...
Report Analyzer::analyze_source(const std::string &source,
                                const std::string &path)
{
    ...
    while (!stack.empty()) {
        ...
        if (strcmp(type, "function_declaration") == 0 ||
            ...
            r.functions++;
            // naive check: count parameters by scanning source substring for
            // commas inside param list
            TSNode params = ts_node_named_child(n, 1); // best-effort
            if (ts_node_is_null(params) == false) {
                uint32_t start           = ts_node_start_byte(params),
                         end             = ts_node_end_byte(params);
                std::string slice        = source.substr(start, end - start);
                int         params_count = 0;
                for (char c : slice)
                    if (c == ',')
                        params_count++;
                // params_count is commas; number of params = commas+ (non-empty
                // ? 1 : 0)
